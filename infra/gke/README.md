...
- backend API
- agent service
- direct OfficeCLI document operations inside the agent
...
The agent invokes the pinned OfficeCLI binary directly against its workspace PVC. There is no Office HTTP service, SSH hop, or independently scaled document layer.
...
| `k8s/50-app.yaml` | Combined backend + agent deployment; the agent image includes OfficeCLI |
