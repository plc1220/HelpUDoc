...
## Sync PVC-backed runtime config (manual)

In GKE, skills are bundled into the backend and agent images and update through CD image deploys. `agent-config-pvc` is still mounted into the running pod; if you change `agent/config/runtime.yaml` in the repo, the cluster can keep using stale PVC contents until you sync it.

After applying manifests (or after a Cloud Build deploy if you're debugging drift), you can sync the runtime config:
