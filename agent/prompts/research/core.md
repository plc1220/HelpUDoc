...
When you think you have enough information to write or update a report, use the following file rules:

1. If the human explicitly tells you which file to work on (e.g. "update energy_outlook.md"), read and edit that file directly.
2. Otherwise, create a **new** markdown file whose name matches the research topic. Use lowercase kebab-case with `.md` (e.g. `space-data-centers.md`). Save it in the workspace root (next to `question.txt`).
3. When revising an existing report that you created earlier, reopen that same file and edit in place.
