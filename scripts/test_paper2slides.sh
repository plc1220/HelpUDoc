#!/usr/bin/env bash
set -euo pipefail

# Simple harness to run paper2slides locally on a markdown file.
# Usage:
#   ./scripts/test_paper2slides.sh [path/to/input.md]
# Defaults to the china-trade-surplus.md fixture.

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DEFAULT_INPUT="$REPO_ROOT/backend/workspaces/b271bf3a-cfda-4ac1-aa83-523d0012ac6a/china-trade-surplus.md"
INPUT_FILE="${1:-$DEFAULT_INPUT}"

if [[ ! -f "$INPUT_FILE" ]]; then
  echo "Input file not found: $INPUT_FILE" >&2
  exit 1
fi

INPUT_DIR="$(cd "$(dirname "$INPUT_FILE")" && pwd)"
PY_BIN="$REPO_ROOT/agent/.venv/bin/python"
[[ -x "$PY_BIN" ]] || PY_BIN="$(command -v python)"

if [[ -z "$PY_BIN" ]]; then
  echo "Python interpreter not found" >&2
  exit 1
fi

export PYTHONPATH="$REPO_ROOT/agent${PYTHONPATH:+:$PYTHONPATH}"

set -x
"$PY_BIN" -m paper2slides \
  --input "$INPUT_FILE" \
  --output slides \
  --content general \
  --style academic \
  --length short \
  --fast \
  --parallel 2 \
  --output-dir "$INPUT_DIR"
