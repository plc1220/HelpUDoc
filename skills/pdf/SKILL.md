...
- Use `search_document` to locate likely pages, then `inspect_document` for a
...
4. Search the original document first.
   - Use `search_document` with the concrete terms implied by the task:
     - summarize
     - extract key findings
     - answer a specific question
     - locate a section, table, or figure reference
   - Inspect the most relevant page ranges with `inspect_document`.
