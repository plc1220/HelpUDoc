...
import { AtSign, Bot, FileText, Plus, Send, Sparkles, Users, X } from 'lucide-react';
...
const Icon = ({ kind }: { kind: TeamChatReference['kind'] }) => kind === 'file' ? <FileText size={14} /> : kind === 'skill' ? <Sparkles size={14} /> : kind === 'agent' ? <Bot size={14} /> : <Users size={14} />;
export default function TeamChatComposer({ options, disabled, sending, reply, onSend }: {
  ...
  const send = async () => {
    ...
    try { await onSend(text, tokens.map((token) => token.reference)); setText(''); setTokens([]); setCaret(0); close(); } catch { /* parent presents error; retain draft */ }
  };
  ...
  return <div className="relative space-y-2">
    ...
    <textarea ref={input} aria-label="Workspace Chat message" aria-controls={open ? 'team-reference-options' : undefined} aria-activedescendant={open && results.length ? `team-reference-${Math.min(active, results.length - 1)}` : undefined}
      value={text} rows={3} disabled={disabled || sending} placeholder={disabled ? 'Viewer access is read-only' : 'Message the team… @people or files, /skills'}
      onSelect={(e) => setCaret(e.currentTarget.selectionStart)} onChange={(e) => { setTokens(reconcileTokens(text, e.target.value, tokens)); setText(e.target.value); setCaret(e.target.selectionStart); setActive(0); setDismissed(''); }} onKeyDown={keyDown}
    ...
    <div className="flex items-center gap-2">
      ...
      <button type="button" disabled={disabled || sending || !text.trim()} onClick={() => void send()} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-2 text-xs text-white disabled:opacity-40"><Send size={14} />{sending ? 'Sending…' : reply ? 'Reply' : 'Send'}</button>
    </div>
  </div>;
}
