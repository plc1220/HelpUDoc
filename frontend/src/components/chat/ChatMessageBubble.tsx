  ...
  return (
    <div className={`group flex items-start gap-3 motion-safe:animate-[chat-pane-message-in_220ms_ease-out] ${isAgentMessage ? '' : 'justify-end'}`}>
      <div style={{ width: '100%', maxWidth: messageBubbleMaxWidth }} className="relative flex-1 md:flex-initial">
        {isAgentMessage ? (
          <div className="w-full rounded-[1.8rem] border border-slate-700/70 bg-[linear-gradient(160deg,rgba(15,23,42,0.98),rgba(30,41,59,0.94))] px-4 py-4 text-slate-100 shadow-[0_26px_70px_-36px_rgba(2,6,23,0.95)] ring-1 ring-white/5">
            <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
              <span className="text-slate-300">{personaDisplayName}</span>
              {timestampLabel ? <span>{timestampLabel}</span> : null}
            </div>
            {displayThinkingText ? (
              <div className="mt-3 rounded-[1.5rem] border border-slate-700/70 bg-slate-950/45 px-3 py-3 text-[13px] text-slate-300 shadow-inner shadow-black/20 transition-all duration-200 ease-in-out">
                  <div className="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">
                    <span className="inline-flex items-center gap-2">
                      <span className="h-2 w-2 rounded-full bg-sky-300 shadow-[0_0_14px_rgba(125,211,252,0.6)]" />
                      Thinking
                    </span>
                    {showThinkingToggle ? (
                      <button
                        ...
                        className="rounded-full border border-slate-700/70 px-2.5 py-1 text-[10px] text-slate-300 transition-all duration-200 hover:border-slate-600 hover:bg-slate-800 hover:text-white"
                      >
            ...
            {hasToolEvents ? (
              <div className="mt-3 rounded-[1.4rem] border border-slate-700/70 bg-slate-950/45 px-3 py-3">
                <button
                  type="button"
                  onClick={() => toggleToolActivityVisibility(message.id)}
                  className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400 transition-all duration-200 hover:text-slate-200"
                >
                  {isToolActivityExpanded ? 'Hide tool activity' : `Show tool activity (${toolEvents.length})`}
                </button>
                {isToolActivityExpanded ? (
                  <div className="mt-3 rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-3 text-xs text-slate-300 shadow-inner shadow-black/15">
                      ...
                      return (
                        <div key={event.id || `${event.name}-${index}`} className="flex min-w-0 gap-3 pb-3 last:pb-0">
                          <div className="flex flex-col items-center">
                            ...
                            {!isLast ? <span className="h-full w-px flex-1 bg-slate-700/80" /> : null}
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className={`break-words text-[11px] font-semibold uppercase tracking-wide ${event.status === 'error' ? 'text-rose-300' : 'text-slate-400'}`}>
                              {event.name}
                            </p>
                            <p className={`whitespace-pre-wrap break-words text-sm ${event.status === 'error' ? 'text-rose-200' : 'text-slate-200'}`}>
                              ...
                              <div className="mt-2 min-w-0 space-y-3">
                                {event.outputFiles.map((file) => (
                                  <div
                                    ...
                                    className="min-w-0 max-w-full overflow-hidden rounded-xl border border-slate-700/70 bg-slate-950/80 p-2"
                                  >
                                    <p className="break-all text-xs font-semibold text-slate-200">{file.path}</p>
          ...
          <div className="rounded-[1.7rem] bg-[linear-gradient(135deg,rgba(56,189,248,0.94),rgba(59,130,246,0.92))] px-4 py-3 text-sm text-white shadow-[0_20px_48px_-28px_rgba(56,189,248,0.75)] [text-shadow:0_1px_1px_rgba(15,23,42,0.35)]">
        ...
        {canCopyMessage ? (
          <button
            ...
            className={`absolute -top-2 ${copyButtonPositionClass} rounded-full border border-slate-700/70 bg-slate-950/95 p-1.5 text-slate-300 shadow ring-1 ring-white/5 transition-all duration-200 opacity-0 hover:bg-slate-900 hover:text-white group-hover:opacity-100 focus-visible:opacity-100`}
          >
            <Copy size={14} />
          </button>
        ) : null}
        {message.sender === 'user' ? (
          <button
            ...
            className={`absolute -right-2 -top-2 rounded-full bg-[linear-gradient(135deg,rgba(56,189,248,0.92),rgba(59,130,246,0.92))] p-1.5 text-white shadow-[0_16px_36px_-22px_rgba(56,189,248,0.85)] transition-all duration-200 opacity-0 ${
              isStreaming
