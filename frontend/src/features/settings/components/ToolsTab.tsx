            ...
            {activeSection === 'mcp' && (
                <div>
                    {!isEditingMcp ? (
                        ...
                        <div className="settings-tools-panel rounded-xl overflow-hidden text-slate-700">
                            <div className="p-6">
                                <div className="flex justify-between items-center mb-6">
                                    <div>
                                        <h3 className="text-xl font-semibold text-slate-900">Connect to a custom MCP</h3>
                                        <p className="text-slate-500 text-sm flex items-center gap-1 hover:text-slate-900 cursor-pointer transition-colors">Docs <ExternalLink size={12} /></p>
                                    </div>
                                    <button onClick={() => setIsEditingMcp(false)} className="text-slate-500 hover:text-slate-900 transition-colors">
                                        <X size={20} />
                                    </button>
                                </div>
                                ...
                                <div className="space-y-5">
                                    <div className="space-y-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm"
                                        />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 bg-slate-100 p-1 rounded-lg">
                                        <button
                                            ...
                                            className={`py-1.5 text-sm font-medium rounded-md transition-all ${mcpForm.transport === 'stdio' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
                                        >
                                            STDIO
                                        </button>
                                        <button
                                            onClick={() => setMcpForm(prev => ({ ...prev, transport: 'http' }))}
                                            className={`py-1.5 text-sm font-medium rounded-md transition-all ${mcpForm.transport === 'http' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
                                        >
                                            Streamable HTTP
                                        </button>
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="block text-sm font-medium text-slate-900">Default access</label>
                                        <div className="grid grid-cols-2 bg-slate-100 p-1 rounded-lg">
                                            <button
                                                onClick={() => setMcpForm(prev => ({ ...prev, defaultAccess: 'allow' }))}
                                                className={`py-1.5 text-sm font-medium rounded-md transition-all ${mcpForm.defaultAccess === 'allow' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
                                            >
                                                Allow
                                            </button>
                                            <button
                                                onClick={() => setMcpForm(prev => ({ ...prev, defaultAccess: 'deny' }))}
                                                className={`py-1.5 text-sm font-medium rounded-md transition-all ${mcpForm.defaultAccess === 'deny' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
                                            >
                                                Deny
                                            </button>
                                        </div>
                                    ...
                                    {mcpForm.transport === 'stdio' ? (
                                        <>
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Command to launch</label>
                                                <input
                                                    type="text"
                                                    value={mcpForm.command}
                                                    onChange={(e) => setMcpForm(prev => ({ ...prev, command: e.target.value }))}
                                                    placeholder="openai-dev-mcp serve-sqlite"
                                                    className="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm font-mono"
                                                />
                                            </div>

                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Arguments</label>
                                                    ...
                                                    {mcpForm.args.map((arg) => (
                                                        <div key={arg.id} className="flex gap-2">
                                                            <input
                                                                ...
                                                                className="flex-1 px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm font-mono"
                                                            />
                                                            <button onClick={() => removeArg(arg.id)} className="p-2 text-slate-500 hover:text-rose-500 transition-colors">
                                                                <Trash2 size={16} />
                                                            </button>
                                                        </div>
                                                    ))}
                                                    <button onClick={addArg} className="w-full py-2 rounded-lg bg-slate-100 text-slate-900 hover:bg-slate-200 text-xs font-medium transition-colors border border-slate-200 flex items-center justify-center gap-2">
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Environment variables</label>
                                                <div className="space-y-2">
                                                    {mcpForm.env.map((env) => (
                                                        <div key={env.id} className="flex gap-2">
                                                            ...
                                                            <input
                                                                ...
                                                                className="flex-1 px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm font-mono"
                                                            />
                                                            <button onClick={() => removeEnv(env.id)} className="p-2 text-slate-500 hover:text-rose-500 transition-colors">
                                                                <Trash2 size={16} />
                                                            </button>
                                                        </div>
                                                    ))}
                                                    <button onClick={addEnv} className="w-full py-2 rounded-lg bg-slate-100 text-slate-900 hover:bg-slate-200 text-xs font-medium transition-colors border border-slate-200 flex items-center justify-center gap-2">
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Environment variable passthrough</label>
                                                <div className="space-y-2">
                                                    {mcpForm.envPassthrough.map((env) => (
                                                        <div key={env.id} className="flex gap-2">
                                                            <input
                                                                ...
                                                                className="flex-1 px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm font-mono"
                                                            />
                                                            <button onClick={() => removeEnvPassthrough(env.id)} className="p-2 text-slate-500 hover:text-rose-500 transition-colors">
                                                                <Trash2 size={16} />
                                                            </button>
                                                        </div>
                                                    ))}
                                                    <button onClick={addEnvPassthrough} className="w-full py-2 rounded-lg bg-slate-100 text-slate-900 hover:bg-slate-200 text-xs font-medium transition-colors border border-slate-200 flex items-center justify-center gap-2">
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Working directory</label>
                                                <input
                                                    type="text"
                                                    value={mcpForm.cwd}
                                        ...
                                        <>
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Bearer token env var</label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Delegated auth provider</label>
                                                <select
                                                    value={mcpForm.delegatedAuthProvider}
                                                    onChange={(e) => setMcpForm(prev => ({ ...prev, delegatedAuthProvider: e.target.value as '' | 'google' }))}
                                                    className="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm"
                                                >
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Headers</label>
                                                <div className="space-y-2">
                                                    {mcpForm.headers.map((env) => (
                                                        <div key={env.id} className="flex gap-2">
                                                            ...
                                                            <input
                                                                ...
                                                                className="flex-1 px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm font-mono"
                                                            />
                                                            <button onClick={() => removeHeader(env.id)} className="p-2 text-slate-500 hover:text-rose-500 transition-colors">
                                                                <Trash2 size={16} />
                                                            </button>
                                                        </div>
                                                    ))}
                                                    <button onClick={addHeader} className="w-full py-2 rounded-lg bg-slate-100 text-slate-900 hover:bg-slate-200 text-xs font-medium transition-colors border border-slate-200 flex items-center justify-center gap-2">
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="block text-sm font-medium text-slate-900">Headers from environment variables</label>
                                                <div className="space-y-2">
                                                    {mcpForm.headersFromEnv.map((env) => (
                                                        <div key={env.id} className="flex gap-2">
                                                            ...
                                                            <input
                                                                ...
                                                                className="flex-1 px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-100 text-slate-900 placeholder-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all text-sm font-mono"
                                                            />
                                                            <button onClick={() => removeHeaderFromEnv(env.id)} className="p-2 text-slate-500 hover:text-rose-500 transition-colors">
                                                                <Trash2 size={16} />
                                                            </button>
                                                        </div>
                                                    ))}
                                                    <button onClick={addHeaderFromEnv} className="w-full py-2 rounded-lg bg-slate-100 text-slate-900 hover:bg-slate-200 text-xs font-medium transition-colors border border-slate-200 flex items-center justify-center gap-2">
                                                        <Plus size={14} /> Add variable
                                                    </button>
                                                </div>
                                            </div>
