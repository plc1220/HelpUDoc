...
const SkillsRegistryTab: React.FC = () => {
  ...
  useEffect(() => {
    if (!selectedSkillId || typeof window === 'undefined') return undefined;
  ...
  return (
    <div style={{ height: 'calc(100vh - 260px)', minHeight: '620px' }} className="settings-workbench flex gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className={`h-full ${workbenchColumnClass}`}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Select a skill to view its guidance, files, and developer settings.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => void loadSkills()}
                  className="settings-portal-button-secondary inline-flex h-9 w-9 items-center justify-center rounded-lg transition-colors"
                  title="Refresh"
            ...
            <div className="space-y-2">
              {filteredSkills.map((skill) => (
                <button
                  ...
                  className="group grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-4 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left transition hover:border-slate-300 hover:bg-slate-50 hover:shadow-sm md:grid-cols-[minmax(0,1fr)_auto_auto]"
                >
                  <div className="flex min-w-0 items-start gap-3">
                    <span className="mt-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-500 transition group-hover:text-emerald-700">
                      <FileText size={17} />
                    </span>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                        <span className="truncate text-sm font-semibold text-slate-900">{skill.name || skill.id}</span>
                        <span className="font-mono text-[11px] text-slate-400">{skill.id}</span>
                        {skill.pluginName && (
                          <span className="inline-flex rounded-md bg-sky-50 px-1.5 py-0.5 text-[11px] font-medium text-sky-700">
                            {skill.pluginName}
                          </span>
                        )}
                      </div>
                      <p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">
                        {skill.description || 'No description has been added yet.'}
                      </p>
                    </div>
                  </div>
                  <span className={`hidden items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs md:inline-flex ${skill.valid
                    ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
                    : 'border-amber-200 bg-amber-50 text-amber-700'
                  }`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${skill.valid ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                    {skill.valid ? 'Ready' : 'Needs attention'}
                  </span>
                  <span className="flex items-center gap-1 text-xs font-medium text-slate-500 transition group-hover:text-slate-800">
                    View details
        ...
        {selectedSkill && (
          <div
            ...
            onMouseDown={(event) => {
              if (event.target === event.currentTarget) setSelectedSkillId(null);
            }}
          >
            <div
              role="dialog"
              ...
              className="settings-modal-panel settings-workbench flex h-[calc(100vh-1.5rem)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl sm:h-[min(88vh,900px)]"
            >
              <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-slate-200 bg-slate-50">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-emerald-700">
                    <FileText size={16} />
                  </span>
                  <div className="min-w-0">
                    <span id="skill-detail-title" className="block truncate text-sm font-semibold text-slate-800">
                      {selectedSkill.name || selectedSkill.id}
                    </span>
                    <span className="block truncate font-mono text-[11px] text-slate-400">{selectedSkill.id}</span>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="settings-portal-button-secondary inline-flex h-8 w-8 items-center justify-center rounded-lg transition"
